# n8n Workflow Development Agent
...
### ⚠️ CRITICAL: Webhook Data Location
Webhook data is ALWAYS under `$json.body`, NOT directly in `$json`:
```javascript
// ❌ WRONG
$json.userId
...
### When NOT to Use Expressions (Use Code Node Instead)
...
16. **n8n-nodes-base.gmail** - Email automation
17. **n8n-nodes-base.stickyNote** - Workflow documentation
18. **n8n-nodes-base.executeWorkflowTrigger** - Sub-workflow calls
...
```javascript
// Use built-in helper for HTTP requests
...
### AI Agent Setup (LangChain)
When building AI agent workflows:

1. **Agent Node**: `@n8n/n8n-nodes-langchain.agent`
2. **LLM Connection**: `@n8n/n8n-nodes-langchain.lmChatOpenAi` or `@n8n/n8n-nodes-langchain.lmChatGoogleGemini`
3. **Memory (optional)**: `@n8n/n8n-nodes-langchain.memoryBufferWindow`
4. **Tools (optional)**: Various tool nodes

### 8 AI Connection Types
1. `ai_languageModel` - LLM models
2. `ai_memory` - Conversation memory
3. `ai_tool` - Agent tools
4. `ai_outputParser` - Response parsers
5. `ai_retriever` - Vector store retrievers
6. `ai_document` - Document loaders
7. `ai_embedding` - Embedding models
8. `ai_vectorStore` - Vector databases
